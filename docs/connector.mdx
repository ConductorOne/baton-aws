...
stringData:
  ...
  # "iam" (default) creates IAM users. "identity-center" creates AWS Identity Center
  ...
  BATON_GLOBAL_AWS_ACCOUNT_PROVISIONING_TARGET: identity-center
...
The `identitystore:CreateUser` and `identitystore:DeleteUser` actions in the SSO policy below are only exercised when `BATON_GLOBAL_AWS_ACCOUNT_PROVISIONING_TARGET=identity-center`. They are listed in the policy regardless so an operator who later switches the target does not need to re-grant the role.
