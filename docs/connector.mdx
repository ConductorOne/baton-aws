...
with an opaque signature error: `BATON_GLOBAL_REGION` and `BATON_GLOBAL_AWS_SSO_REGION` must be in
the same partition as `BATON_ROLE_ARN`, and `BATON_GLOBAL_ROLE_ARN` (two-hop mode) must be in that
partition too.
